---
title: 'AstroEduPlanner (AEOP): An Open-Source Educational Astronomical Observation Planner'
tags:
  - Python
  - astronomy
  - education
...
Several established tools address observation planning for professional or advanced amateur use, but none combines a no-code interface with a programmable, pedagogically structured backend. Stellarium [@Stellarium2023] and Cartes du Ciel provide rich sky visualisation but offer no programmatic API and cannot be scripted for classroom assignments. Skyfield provides precise ephemerides as a Python library but has no built-in educational interface and presupposes substantial programming experience. Astroplan [@Morris2018] targets professional observatory scheduling and, while powerful, lacks a user-friendly interface accessible to students with no programming background.

AEOP occupies a distinct niche by spanning this spectrum: it pairs a no-code web interface (Streamlit) with a programmable Jupyter Notebook, both built on the same modular scientific backend. This makes it appropriate for a continuum of users from secondary-school students with no coding experience to undergraduates learning the Astropy ecosystem, an audience and use case not served as a single integrated tool by the existing software listed above.

# Software Design

AEOP is organised as a modular Python package whose scientific logic is fully decoupled from its two user-facing interfaces. The backend consists of five independent modules in `src/`: `config.py` (centralised configuration and preset target lists), `location.py` (geolocation via Nominatim and coordinate handling), `targets.py` (target management and SIMBAD/Astroquery resolution with a `SkyCoord.from_name()` fallback), `analysis.py` (visibility, airmass, lunar, and Solar System computations using Astropy's `AltAz` frame transformations and `get_body()`), and `plotting.py` (all visualisation routines). All user-visible strings are centralised in a single `src/i18n/strings.py` module containing 62 English/Portuguese string pairs, so that adding a new language is a localised, non-invasive change.

The two interfaces consume this shared backend without duplicating logic. The Streamlit application (`app.py`) wraps the backend in a graphical sidebar requiring no programming, while the Jupyter Notebook (`analise_astronomica.ipynb`) exposes the same functions for inspection and extension. This separation of concerns was deliberately chosen to align with software-engineering concepts typically introduced in second-year undergraduate curricula. The project ships with a pytest test suite covering the analysis, location, and target modules; continuous integration via GitHub Actions; dependency management through `requirements.txt` and `pyproject.toml`; and machine-readable citation metadata in `CITATION.cff`.
...
AEOP's intended impact is on astronomy and scientific-computing education rather than on the production of new research data. By removing the technical and linguistic barriers to pre-observation planning, the tool enables instructors to devote scarce classroom and observatory time to astronomical reasoning instead of software operation. The dual-interface design supports a deliberate learning progression — from no-code exploration to inspection and extension of a documented Python codebase — that scaffolds students into the broader Astropy ecosystem and into general scientific-software practices such as modular design, unit testing, and version control.

The software is expected to be most impactful in under-resourced contexts, particularly Brazilian public secondary schools and regional universities, where commercial planning platforms are financially or technically inaccessible and where an English-only interface would itself exclude many learners. By foregrounding southern-hemisphere targets and providing a fully Portuguese interface, AEOP aims to make practical observation planning attainable for educational communities that existing tools have not served, while remaining fully usable and citable by the international astronomy-education community in English.

# AI Usage Disclosure

Generative AI tools were used in a limited, assistive capacity during the preparation of this work. AI assistance was used to help draft and copy-edit portions of the manuscript text and software documentation, and to suggest refactorings and docstrings during code development. All scientific content, the software architecture and algorithms, the test suite, and all final text were designed, written, reviewed, and verified by the authors, who take full responsibility for the correctness and integrity of the software and the paper. No AI-generated content was incorporated without author review.
